...
public abstract class WebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
	...
	protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
    	...
    	if (msg instanceof WebSocketFrame) {
			...
			handleRpcRequest(ctx, (RpcMessage) msg);
		} else
...
//            HttpUtil.setContentLength(res, content.readableBytes());
    ...
    private void handleBinaryWebSocketFrame(ChannelHandlerContext ctx, BinaryWebSocketFrame frame) {
    	try {
			...
			RpcMessage rpc = RpcMessage.parseFrom(bytes);
			handleRpcRequest(ctx, rpc);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
    }
    
    protected abstract void handleRpcRequest(ChannelHandlerContext ctx, RpcMessage msg);
...
//            HttpUtil.setContentLength(res, res.content().readableBytes());
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
    	...
    	Session session = PokerMananger.removeSession(ctx.channel());
    	if(session != null) {
    		session.release();
    	}
    }
